...
const events=['order_submitted','payment_approved','payment_rejected','order_cancelled','order_expired','order_updated','fulfillment_reminder','ready_for_pickup','pickup_reminder','out_for_delivery','delivery_tracking_updated','delivery_tracking_replaced','delivery_tracking_removed','order_review_required'];
const messages=[];
for(const event of events){
 const trackingChange=({delivery_tracking_updated:'added',delivery_tracking_replaced:'replaced',delivery_tracking_removed:'removed'})[event];
 const delivery=event==='out_for_delivery'||Boolean(trackingChange);
 const trackingUrl=delivery&&trackingChange!=='removed'?'https://tracking.example.test/ELIO-PREVIEW':null;
 let {html,text}=renderEmail({event_type:trackingChange?'delivery_tracking_updated':event,order:{...order,method:delivery?'delivery':'pickup',delivery_tracking_url:trackingUrl},settings,product_photos:photos,reason:'Preview only — no actual order or payment.',tracking_change:trackingChange,tracking_url:trackingUrl,previous_tracking_url:trackingChange&&trackingChange!=='added'?'https://tracking.example.test/PREVIOUS-PREVIEW':null});
 ...
 html=html.replace(/href="https:\/\/tracking.example.test\/[^\"]*"/g,'href="#preview-only"');text=text.replace(/https:\/\/tracking.example.test\/[^\s]+/g,'[inactive courier tracking preview]');
