...
export function emailColumns(left: string, right: string): string {
 return `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:24px 0 0;border-top:1px solid #dfd1bd"><tr><td class="email-column" width="55%" valign="top" style="padding:20px 24px 0 0;overflow-wrap:anywhere;word-break:break-word">${left}</td><td class="email-column" width="45%" valign="top" style="padding:20px 0 0;overflow-wrap:anywhere;word-break:break-word">${right}</td></tr></table>`;
}
...
export function emailProducts(items: any[], photos: any, site: URL, labels: (item: any) => string[], money: (value: unknown) => string): string {
 return emailSection("Inside your order", `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">${items.map(item => {
  const photo = productPhoto(item, photos, site);
  const parts = labels(item);
  return `<tr>${photo ? `<td class="email-photo-cell" width="116" valign="top" style="width:116px;padding:16px 16px 16px 0;border-top:1px solid #e7ddce"><img class="email-photo" src="${e(photo)}" width="100" alt="${e(item.name)}" style="display:block;width:100px;max-width:100%;height:auto;border:0;border-radius:4px"></td>` : ""}<td valign="top" style="padding:16px 0;border-top:1px solid #e7ddce;overflow-wrap:anywhere;word-break:break-word"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"><tr><td class="email-product-name" valign="top" style="padding:0 12px 7px 0"><h3 style="margin:0;font:bold 15px/1.5 Arial,sans-serif;color:#39251c">${e(item.quantity)} × ${e(item.name)}</h3></td><td class="email-item-price" width="90" align="right" valign="top" style="width:90px;padding:2px 0 7px;font-size:14px;font-weight:bold;white-space:nowrap;color:#39251c">${e(money(item.line_total_cents ?? item.quantity * item.unit_price_cents))}</td></tr></table>${parts.map(label => `<span style="display:inline-block;max-width:100%;margin:0 4px 4px 0;padding:4px 7px;background:#f2eadc;border:1px solid #e7dac5;border-radius:3px;font:12px/1.5 Arial,sans-serif;color:#63412d">${e(label)}</span>`).join("")}${parts.length ? '<p style="margin:3px 0 0;font-size:11px;line-height:1.5;color:#786858">In each box</p>' : ""}</td></tr>`;
