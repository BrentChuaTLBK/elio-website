...
export function cors(request: Request): Headers {
  ...
  if (origin) {
    const allowed = (env("ALLOWED_ORIGINS") || "https://eliocheesecakes.com,https://www.eliocheesecakes.com,http://127.0.0.1:4173,http://localhost:4173").split(",").map(x => x.trim()).filter(Boolean);
