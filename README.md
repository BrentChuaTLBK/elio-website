...
- The carousel glides automatically at a gentle 14 pixels per second, with the first flavor following the last seamlessly. It keeps moving while the cursor is over it, with only previous/next arrows in the controls. Native touch/trackpad scrolling and keyboard arrows and Home/End remain available.
...
Automatic movement continues on hover. It pauses while swiping, while details are open, and when the carousel or page is out of view. It resumes automatically four seconds after manual navigation or focus interaction. Reduced-motion preferences disable automatic movement. Screen-reader status announcements are silent while it moves automatically; the full catalog page provides a stationary view of all flavors.
...
Slow autoplay was checked in Chrome at 1440, 390, and 320 px, including measured speed, seamless wrapping, continued movement on hover, automatic resumption after navigation, reduced-motion preferences, and control spacing. Native phone swipes pause the glide and resume it after the interaction delay. Offscreen and open-dialog pauses also passed. The existing carousel regression checks and 32 consecutive native swipes in each direction at both phone widths passed again.
