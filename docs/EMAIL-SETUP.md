...
Run `node --experimental-transform-types scripts/preview-emails.mjs <output-directory>` to generate a navigable gallery of 27 designs: 11 order templates, 13 account templates, and three newsletter drafts. The extra pickup-reminder template does not enable a new scheduler. Newsletter previews do not enable signup, sending, discounts or campaigns; their offer and mailing-address placeholders need real configuration before use. The gallery's account preview links are inactive. `supabase-auth-templates.json` contains the real Auth placeholders for a narrowly scoped Management API patch, but generating it does not change hosted configuration.
...
On September 26, 2026, an authorized live test verified guest checkout, private proof upload, manual payment approval, production counts, confirmation delivery, and cancellation with stock restored. All four order/staff emails were delivered. A separate signup verified the previously saved branded Auth template; the unconfirmed test account was removed. The new dark-header Auth templates are prepared, but subsequent delivered tests still showed the earlier signup template and plain recovery email despite reported dashboard saves. The connected tools cannot inspect or patch hosted Auth configuration. Auth logs showed no template parsing errors in the tested window; activation remains unverified.

A live forgot-password acceptance test used a separate, temporary verified customer account. Email delivery, opening a recovery link in a fresh browser, minimum password length, confirmation matching, saving the replacement password, old-password rejection, new-password sign-in, and single-use recovery links all passed. No owner password was changed. The temporary account was removed, including its test sessions.

The final reference-based gallery was checked in Chrome across 81 layouts (27 templates at 900, 390 and 320 pixels), including image loading, blocked images, column stacking and gallery navigation. This is not a full Outlook or Apple Mail client certification.
