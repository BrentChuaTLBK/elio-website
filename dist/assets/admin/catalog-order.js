import { catalogProductGroups, sortProducts } from './catalog-ordering.js';
...
export function mountCatalogOrder(root, { kind, scope, items, categories, editable, api, escapeHtml:esc, safeImage, onSaved }) {
  if (kind==='products') items=sortProducts(items);
  ...
  const makeGroups=items=>[{id:'*',name:scope==='flavors'?'All flavors':'All boxes',items:sortProducts(items)},...catalogProductGroups(items,categories).filter(g=>g.id)];
  const groups=product?makeGroups(items):[{id:'categories',name:'Categories',items}];
  ...
  root.addEventListener('click',async event=>{
    ...
    try {
      const result=product
        ? await api('reorder_catalog',{kind,scope,groups:groups.map((group,i)=>({category_id:group.id,ids:draftGroupOrders[i]})),expected:snapshot()})
        : await api('reorder_catalog',{kind,scope,ids:draft.map(item=>item.id),expected:snapshot()});
      saved=structuredClone(result.items); draft=structuredClone(result.items);
      if (product) { savedGroupOrders=groups.map(group=>makeGroups(saved).find(next=>next.id===group.id)?.items.map(item=>item.id)||[]); draftGroupOrders=structuredClone(savedGroupOrders); }
      paint();
