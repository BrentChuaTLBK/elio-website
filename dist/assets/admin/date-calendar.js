...
function calendarView(name, month, selected, today, focusDate, disabled, options = {}) {
  ...
  const production = name === 'nonproduction_dates';
  const closedLabel = production ? 'No production' : 'Closed';
  ...
  for (let index = 0; index < cells.length; index += 7) {
    rows.push(`<tr>${cells.slice(index, index + 7).map(date => date ? `<td><button type="button" class="calendar-day" data-calendar-date="${date}" aria-label="${dateLabel(date)}${closures ? selectedSet.has(date) ? `, ${closedLabel.toLowerCase()}; click to restore regular schedule` : `, regular schedule; click to ${production ? 'mark as a day off' : 'close this date'}` : ''}" aria-pressed="${selectedSet.has(date)}" ${date === today ? 'aria-current="date"' : ''} tabindex="${date === tabDate ? 0 : -1}" ${disabled || (options.minDate && date < options.minDate) ? 'disabled' : ''}><span class="calendar-day-number">${Number(date.slice(8))}</span></button></td>` : '<td></td>').join('')}</tr>`);
  }
  if (closures) {
    const count = selected.filter(date => date.startsWith(month)).length;
    return `<div class="calendar-toolbar"><button type="button" class="calendar-nav" data-calendar-move="-1" aria-label="Previous month">‹</button><strong id="calendar-${escape(name)}-month" aria-live="polite">${monthLabel(month)}</strong><button type="button" class="calendar-nav" data-calendar-move="1" aria-label="Next month">›</button></div><table class="calendar-month" aria-labelledby="calendar-${escape(name)}-title calendar-${escape(name)}-month"><thead><tr>${WEEKDAYS.map(day => `<th scope="col">${day}</th>`).join('')}</tr></thead><tbody>${rows.join('')}</tbody></table><div class="calendar-footer calendar-closure-legend"><span><i class="calendar-open-key" aria-hidden="true"></i> Regular schedule</span><span><i class="calendar-closed-key" aria-hidden="true"></i> ${closedLabel}</span><button type="button" class="calendar-today" data-calendar-today>Current month</button></div><p class="calendar-closure-summary">${count ? `${count} extra ${production ? `day${count === 1 ? '' : 's'} off` : `closed date${count === 1 ? '' : 's'}`} this month` : `No extra ${production ? 'days off' : 'closed dates'} this month`}</p>`;
  }
  return `<div class="calendar-toolbar"><button type="button" class="calendar-nav" data-calendar-move="-1" aria-label="Previous month" ${options.fixedMonth ? 'disabled' : ''}>‹</button><strong id="calendar-${escape(name)}-month" aria-live="polite">${monthLabel(month)}</strong><button type="button" class="calendar-nav" data-calendar-move="1" aria-label="Next month" ${options.fixedMonth ? 'disabled' : ''}>›</button></div><table class="calendar-month" aria-labelledby="calendar-${escape(name)}-title calendar-${escape(name)}-month"><thead><tr>${WEEKDAYS.map(day => `<th scope="col">${day}</th>`).join('')}</tr></thead><tbody>${rows.join('')}</tbody></table><div class="calendar-footer"><span><span class="calendar-selected-key" aria-hidden="true">✓</span> ${escape(options.selectionLabel || (name === 'nonproduction_dates' ? 'Selected non-production dates' : 'Selected dates are closed'))}</span><button type="button" class="calendar-today" data-calendar-today ${options.fixedMonth ? 'hidden' : ''}>Current month</button></div>${options.mode==='single'?'':`<details class="calendar-selection" ${selected.length > 0 && selected.length <= 6 ? 'open' : ''}><summary>${selected.length} selected date${selected.length === 1 ? '' : 's'}</summary>${selected.length ? `<div class="calendar-date-list">${selected.map(date => `<button type="button" data-calendar-remove="${date}" aria-label="Remove ${dateLabel(date)}" ${disabled ? 'disabled' : ''}>${dateLabel(date)} <span aria-hidden="true">×</span></button>`).join('')}</div>` : '<p>No additional dates selected.</p>'}</details>`}`;
}
...
export function dateCalendar(name, title, values, hint, today, disabled = false, options = {}) {
  ...
  const month = options.fixedMonth || ((options.minDate || options.mode==='single') && selected[0] ? selected[0] : today).slice(0, 7);
  const instructions = options.mode==='closures' ? 'Click a date to cross it out; click again to uncross it. Weekday settings still apply.' : options.mode==='single' ? '' : 'Select a date to mark it; select it again to remove it.';
  return `<fieldset class="date-calendar${options.mode==='closures'?' date-calendar-closures':''}" data-date-calendar data-mode="${escape(options.mode||'multiple')}" data-month="${month}" data-today="${today}" data-disabled="${disabled}" data-save-label="${escape(options.saveLabel || 'Save shop settings')}" data-selection-label="${escape(options.selectionLabel || '')}" data-min-date="${escape(options.minDate || '')}" data-fixed-month="${escape(options.fixedMonth || '')}"><legend id="calendar-${escape(name)}-title">${escape(title)}</legend><p class="calendar-hint">${escape(hint)} ${instructions} Click ${escape(options.saveLabel || 'Save shop settings')} when finished.</p><textarea name="${escape(name)}" hidden>${selected.join('\n')}</textarea><div data-calendar-view>${calendarView(name, month, selected, today, today, disabled, options)}</div><p class="sr-only" aria-live="polite" data-calendar-status></p></fieldset>`;
}
...
export function bindDateCalendars(root) {
  const render = (calendar, { month, selected, focusDate, focusSelector } = {}) => {
    ...
    calendar.querySelector('[data-calendar-view]').innerHTML = calendarView(field.name, calendar.dataset.month, dates, calendar.dataset.today, focusDate, calendar.dataset.disabled === 'true', { selectionLabel: calendar.dataset.selectionLabel, minDate: calendar.dataset.minDate, fixedMonth: calendar.dataset.fixedMonth, mode: calendar.dataset.mode });
  ...
  root.addEventListener('click', event => {
    ...
    if (button.hasAttribute('data-calendar-move')) {
      ...
      const dates = calendar.dataset.mode==='single' ? [date] : toggleCalendarDate(selected, date);
      render(calendar, { selected: dates, ...(button.dataset.calendarDate ? { focusDate: date } : { focusSelector: '.calendar-selection summary' }) });
      calendar.querySelector('[data-calendar-status]').textContent = calendar.dataset.mode==='closures'
        ? `${dateLabel(date)} ${dates.includes(date) ? field.name==='nonproduction_dates' ? 'marked as a day off' : 'closed' : 'restored to the regular weekday schedule'}. ${calendar.dataset.saveLabel} to apply.`
  ...
  root.addEventListener('keydown', event => {
    ...
    if (calendar.dataset.fixedMonth && !date.startsWith(calendar.dataset.fixedMonth)) return;
