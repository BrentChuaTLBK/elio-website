...
let supabase = null;
// Preview only renders public draft content. It never authenticates or calls the API.
try { configured = currentUrl.searchParams.get('preview') !== '1' && publicConfiguration(); } catch (error) { initializationError = error; }
...
export const ready = (async () => {
  ...
  try {
    ...
    supabase = createClient(config.supabaseUrl, config.supabasePublishableKey, {
      auth: { persistSession: true, autoRefreshToken: true, detectSessionInUrl: true, flowType: 'implicit', storageKey: 'elio-auth-v1' },
    });
    auth = supabase.auth;
