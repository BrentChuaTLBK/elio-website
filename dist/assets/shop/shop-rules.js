...
export function customerDateIssue(date,settings={},method='pickup',now=new Date(),allowSameDay=false){
  ...
  if(date>window.maxDate)return `Bookings are open through ${window.maxDate}. Choose a date in the current month or next month.`;
...
export function sameDayBasketEligible(items,products,inventory,method='pickup',now=new Date()){
  ...
  return products.some(p=>{if(!eligible(p))return false;return boxStock(p,today,inventory)>=Number(p.min_quantity||1)});
}
export function availability(product,date,settings,inventory,now=new Date(),method='pickup'){
  ...
  const pieces = boxStock(product,date,inventory);
  if(pieces < Number(product.min_quantity||1))return {available:false,remaining:pieces,reason:date?'Sold out for this date':'An included flavor is unavailable',earliest};
  ...
  if(!Number.isFinite(pieces))return {available:true,unlimited:true,remaining:Infinity,reason:'Available on this date',earliest};
...
export function selectionPrice(product,selections){
  if(product.kind==='set'&&Object.keys(selections||{}).length)throw new Error('Fixed-set flavors cannot be changed.');
...
export function selectionLabels(product,selections){
  if(product.kind==='set')return (product.flavor_contents||[]).map(f=>`${f.quantity} × ${f.name}`);
  return (product.option_groups||[]).flatMap(group=>group.choices.filter(c=>Number(selections[group.id]?.[c.id])>0).map(c=>`${selections[group.id][c.id]} × ${c.label}${c.surcharge_cents ? ` (+PHP ${(c.surcharge_cents/100).toFixed(2)} each; PHP ${(c.surcharge_cents*selections[group.id][c.id]/100).toFixed(2)} per unit)` : ''}`));
}
...
export function boxStock(product,date,inventory){
  if(product.kind==='set'){
    const recipe=product.flavor_contents||[];
    if(recipe.reduce((sum,f)=>sum+f.quantity,0)!==3)return 0;
    return Math.min(...recipe.map(f=>Math.floor(flavorStock(f,date,inventory)/f.quantity)));
  }
  const choices=product.option_groups?.find(g=>g.id==='flavors')?.choices||[];
  return Math.floor(choices.reduce((sum,f)=>sum+flavorStock(f,date,inventory),0)/3);
}
export function basketStockIssues(items,products,inventory,date){
  const demand=new Map(),flavors=new Map();
  for(const line of items){
    const p=products.find(p=>p.id===line.product_id);if(!p)continue;
    const recipe=p.kind==='set'?p.flavor_contents||[]:Object.entries(line.selections?.flavors||{}).map(([id,quantity])=>({...(p.option_groups?.[0]?.choices.find(f=>f.id===id)||{id,active:false}),quantity}));
    for(const f of recipe){const id=f.product_id||f.id;demand.set(id,(demand.get(id)||0)+f.quantity*line.quantity);flavors.set(id,f);}
  }
  return [...demand].flatMap(([id,quantity])=>{
    const f=flavors.get(id),remaining=flavorStock(f,date,inventory);
    return quantity>remaining?[`${f.name||f.label||'A flavor'}: only ${remaining} pieces available${date?' for this date':''}. Adjust your basket or choose another date.`]:[];
  });
}
export function firstAvailableDate(items,products,settings,inventory,now=new Date()){
  if(settings.paused)return '';
  const {today,maxDate}=customerBookingWindow(now,true);
  for(let date=today;date<=maxDate;date=addDays(date,1)){
    if(items.length){
      const valid=items.every(line=>{
        const p=products.find(p=>p.id===line.product_id);if(!p)return false;
        try{selectionPrice(p,line.selections||{});}catch{return false;}
        return availability(p,date,settings,inventory,now,'pickup').available;
      });
      if(valid&&!basketStockIssues(items,products,inventory,date).length)return date;
    }else if(products.some(p=>availability(p,date,settings,inventory,now,'pickup').available))return date;
  }
  return '';
}
